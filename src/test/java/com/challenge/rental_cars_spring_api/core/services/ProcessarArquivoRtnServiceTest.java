...
import com.challenge.rental_cars_spring_api.core.service.LinhaRtnParser;
import com.challenge.rental_cars_spring_api.core.service.ProcessarArquivoRtnService;
...
class ProcessarArquivoRtnServiceTest {
    ...
    void deveIgnorarLinhaInvalidaDoArquivo() throws IOException {
        String linhaInvalida = "LINHA_INVALIDA";
        String conteudoInvalido = linhaInvalida + "\n" + conteudoArquivo;
        MockMultipartFile arquivo = new MockMultipartFile(
            "arquivo.rtn",
            "arquivo.rtn",
            "text/plain",
        ...
        when(linhaRtnParser.processarLinha(eq(linhaInvalida))).thenReturn(null);
        when(linhaRtnParser.processarLinha(eq(conteudoArquivo))).thenReturn(aluguelValido);
        when(aluguelRepository.salvarTodos(anyList())).thenReturn(List.of(aluguelValido));

        service.processar(arquivo);

        verify(aluguelRepository, times(1)).salvarTodos(anyList());
        verify(linhaRtnParser, times(1)).processarLinha(eq(linhaInvalida));
        verify(linhaRtnParser, times(1)).processarLinha(eq(conteudoArquivo));
    }

    @Test
    void deveProcessarArquivoVazio() throws IOException {
        MockMultipartFile arquivo = new MockMultipartFile(
            "arquivo.rtn",
            "arquivo.rtn",
            "text/plain",
